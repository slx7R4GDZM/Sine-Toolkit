...
Settings_Handler::Settings_Handler()
    : button_key{Kb::E, Kb::F, Kb::W, Kb::S, Kb::A, Kb::D, Kb::Q, Kb::F11, Kb::Escape}
...
void Settings_Handler::parse_file_settings(std::ifstream& input)
{
    ...
    while (getline(input, line))
    {
        if (!line.empty() && line[0] != '#')
        {
            ...
            for (unsigned int i = 0, option = 0; i < line.length(); i++)
            {
                if (line[i] == ' ') // setting
                {
...
void Settings_Handler::parse_buttons(const string& setting, const string& value)
{
    ...
    for (int i = 0; i < TOTAL_BUTTONS && current_button == INVALID_BUTTON; i++)
    {
    ...
    if (current_button != INVALID_BUTTON)
    {
        ...
        for (int i = 0; i < TOTAL_KEYS && key == Kb::Unknown; i++)
        {
...
{
    if (setting == "Starting-Window-Mode")
...
void Settings_Handler::create_window(RenderWindow& win)
{
    ...
    win.create(sf::VideoMode(current_res.x, current_res.y), "Sine Toolkit", window_style, context_settings);
